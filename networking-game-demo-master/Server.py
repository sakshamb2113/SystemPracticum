...
server = "127.0.0.1"    # localhost binding is fine
port = 5003
...
class GameSocket:
    ...
    def __init__(self, ip, port, admin_ip):
    ...
    def process_payload(self, data, addr):
        # if the server is full, send back that message and disconnect.
        if self.total_players >= MAX_CLIENTS:
            limit_message = {"status": "max_connection"}
        ...
        payload = {
            "status": "connected",
            "player_id": player_id,
            "player_states": self.filter_players(players)
        }
        ...
        while True:
            ...
            for i in range(len(self.clients_time)):
                if self.clients_time[i] is not None:
                    if current - self.clients_time[i] > TIMEOUT_SECONDS * 1000000000:
    ...
    def process_state(self, command, player_id):
        ...
        self.players[player_id].move(command, GAME_SIZE)
        self.state_lock.release()
